...
import '../../services/updated_payment_history_service.dart';
  ...
  Future<void> _loadPaymentHistory() async {
    ...
    try {
      ...
      if (result['success']) {
        ...
        // **ENHANCED: Separate actual payments from overdue entries**
        final actualPayments = paymentHistory.where((t) => t['isActualPayment'] == true).toList();
        final overduePayments = paymentHistory.where((t) => t['isActualPayment'] == false).toList();
  ...
  void _onTransactionTap(Map<String, dynamic> transaction) {
    ...
    showDialog(
      ...
      builder: (context) => AlertDialog(
        ...
        content: SingleChildScrollView(
          child: Column(
            ...
            children: [
              ...
              if (!isPending && !isOverdue) _buildDetailRow('Method', transaction["paymentMethod"]),
              _buildDetailRow('Status', transaction["status"]),
              _buildDetailRow('Description', transaction["description"]),
              _buildDetailRow(
                  isPending || isOverdue ? 'Due Date' : 'Date',
        ...
        actions: [
          if ((isPending || isOverdue) && status.toLowerCase() != 'paid')
            ElevatedButton.icon(
              onPressed: () {
              ...
              icon: Icon(
                isOverdue ? Icons.priority_high : Icons.payment,
                color: Colors.white,
              ),
              label: Text(isOverdue ? 'Pay Now!' : 'Pay Now'),
