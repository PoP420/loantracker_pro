...
class PaymentHistoryService {
  ...
  static Future<Map<String, dynamic>> getUserPaymentStatusHistory(
      int userId) async {
    try {
      print('=== FETCHING PAYMENT STATUS HISTORY ===');
      print('User ID: $userId');

      final uri =
          Uri.parse('$_baseUrl/Loans/user/$userId/payment-status-history');
      ...
      print('Payment Status History Response Status: ${response.statusCode}');
      print('Payment Status History Response Body: ${response.body}');

      if (response.statusCode == 200) {
        final List<dynamic> historyData = jsonDecode(response.body);

        final paymentHistory = historyData.map((payment) {
          String displayStatus = payment['status'] as String? ?? 'Unknown';
          double amount = (payment['amount'] as num?)?.toDouble() ?? 0.0;

          // Debug logging to verify field mapping
          print('Mapping payment: amount=${amount}, status=${displayStatus}');
        ...
        paymentHistory.sort((a, b) => (b['paymentDate'] as DateTime)
        ...
        return {
          'success': false,
          'message':
              errorData['Message'] ?? 'No payment history found for this user',
        };
      } else {
        final errorData = jsonDecode(response.body);
        return {
      ...
      print('Payment Status History Error: $e');
      return {
        'success': false,
      ...
      if (response.statusCode == 200) {
        ...
        final paymentHistory = historyData.map((payment) {
          String displayStatus =
              _mapPaymentStatus(payment['status'] as String?);
          ...
          return {
            ...
            'isPending': displayStatus.toLowerCase().contains('pending') ||
                displayStatus.toLowerCase() == 'scheduled',
            'isOverdue': displayStatus.toLowerCase() == 'overdue',
            'isScheduled': displayStatus.toLowerCase() == 'scheduled',
          };
        }).toList();
      ...
      if (result['success']) {
        ...
        return {
          ...
          'statistics': {
            'totalPaid': totalPaid,
            'totalPayments':
                payments.where((p) => p['isActualPayment'] == true).length,
  ...
  static Future<Map<String, dynamic>> getRecentPaymentStatusHistory(
      int userId) async {
    try {
      final result = await getUserPaymentStatusHistory(userId);
      ...
      return {
        'success': false,
        'message':
            'Failed to fetch recent payment status history: ${e.toString()}',
      };
    }
  }
}
